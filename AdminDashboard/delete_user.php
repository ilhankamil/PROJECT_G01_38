...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['username'])) {
    ...
    if ($user) {
        ...
        if ($userType == 'customer') {
            ...
            // Delete user data from the staff table
            $deleteStaffStmt = $pdo->prepare("DELETE FROM staff WHERE username = :username");
            $deleteStaffStmt->bindParam(':username', $username, PDO::PARAM_STR);
            ...
            if ($deleteStaffStmt->execute()) {
                ...
                if ($deleteUserStmt->execute()) {
                    // User deleted successfully
                    $response['success'] = true;
                    $response['message'] = "Staff, " . $username . " has been deleted.";
                } else {
                    // Error handling
                    $response['success'] = false;
                    $response['message'] = "Error deleting staff user.";
                }
            } else {
                // Error handling
                $response['success'] = false;
                $response['message'] = "Error deleting staff user data.";
            }
        } else {
            // Invalid user type
            $response['success'] = false;
